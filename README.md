...
一个面向手机和折叠屏优化的美股、港股、A 股个人持仓看板。项目采用 React + TypeScript 前端和轻量 Node.js 后端，支持账户总览、持仓拆分、实时行情、收益日历、趋势图、手动买卖登记、OCR 与手动持仓导入、持仓新闻、走势预判和 AI 问询。
...
- 合并持仓展示今日盈亏与仓位占比
- 仓位结构饼图：多色扇区、外侧标签与占比；权重约 10 分钟刷新，标题显示「10分钟刷新」
- **收益日历**（底部导航「持仓」后的 **收益** 页）
  - 日 / 周 / 月 / 年视图；单位默认 **美元**，可切换 **百分比**
  - 日历格盈绿亏红；日期数字黑色，收益数值着色；选中日深灰底
  - 点击日期或周期后，下方展示 **当日/当期收益明细**（分标的金额、收益率、股数）
  - 仅从首次买入 / 导入 / 快照日起估算，不把当前仓位套到更早月份
  - 今日优先实时今日盈亏；历史日用日线 × 可还原仓位（非正式对账单）
  - 开启顶部「隐藏」时，金额与明细中的股数一并遮罩
- 手工登记买入、加权成本、卖出、已实现盈亏和撤销审计；市场支持 **美股 / 港股 / 大A**（CNY）
- A 股代码自动规范为 `xxxxxx.SH` / `xxxxxx.SZ`，行情走腾讯接口
- 新增买入 / 手动导入支持 **新增券商名称**（自定义券商，不限预设列表）；已用过的券商会自动出现在下拉中
- 券商截图 OCR 识别、逐行确认和账户级增量更新（预设券商截图优先；截图中其它券商名可原样保留）
...
- 合并持仓表在折叠/窄栏下可 **左右滑动** 查看全部列；名称列吸左
- 手机、折叠屏和电脑响应式布局
...
买入登记与手动导入还支持 **任意自定义券商名称**（下拉「新增券商名称…」）。
