...
export function HoldingNewsBoard({
  ...
  return (
    <section className={`panel news-board ${compact ? 'compact' : ''}`}>
      <PanelTitle icon={Newspaper} title="持仓相关新闻" action={fetchedAt ? `最后更新 ${formatFetchedAt(fetchedAt)}` : statusLabel[status]} />
